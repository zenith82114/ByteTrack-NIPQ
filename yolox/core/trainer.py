...
class Trainer:
    ...
    def train_one_iter(self):
        ...
        if self.args.nipq:
            beta = 1.0
            bit_loss = Q.bit_loss(self.model, self.epoch, self.args.a_scale, self.args.w_scale, self.args.target, True)
            loss = loss + beta * bit_loss
            outputs["bit_loss"] = bit_loss
    ...
    def before_train(self):
        ...
        # solver related init
        self.optimizer = self.exp.get_optimizer(self.args.batch_size, self.args.nipq)

        # value of epoch will be set in `resume_train`
        # state_dict will also be loaded here
        model = self.resume_train(model)

        if self.args.pruning:
            pruner = init_pruner(model, self.args, None)
            model = pruner.prune()
        ...
        # nipq init
        if self.args.nipq:
            Q.initialize(model, act=self.args.a_scale > 0, weight=self.args.w_scale > 0)
            ...
            for name, module in model.named_modules():
                if isinstance(module, (Q.ReLU, Q.SiLU, Q.Sym, Q.HSwish)):
